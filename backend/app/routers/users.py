...
async def list_users(
    ...
    comps = {c["id"]: c["name"] for c in await access.companies.find({}).to_list(1000)}
...
async def create_user(
    ...
    if body.role == "customer":
        if body.newCompany and body.newCompany.name.strip():
            ...
            c = await access.companies.find_one({"id": body.companyId})
            if not c:
    ...
    await global_audit(admin, "user.create", uid, {"role": body.role, "email": email})
...
async def admin_reset_password(user_id: str, admin: Annotated[dict, Depends(require_roles("admin"))]):
    ...
    await global_audit(admin, "user.reset", user_id, {"email": u["email"]})
