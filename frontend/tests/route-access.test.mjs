...
test("admin may open all internal route classes directly", () => {
  for (const path of [
    "/",
    "/produkte", "/benutzer", "/abos", "/audit", "/shop-admin", "/maschinen-admin", "/einstellungen", "/systembetrieb",
...
test("sales direct URLs allow staff workflows and reject admin-only routes", () => {
  ...
  for (const path of ["/benutzer", "/produkte", "/abos", "/audit", "/shop-admin", "/maschinen-admin", "/einstellungen", "/systembetrieb", "/vertrieb/u1"])
